...
def fish(request):
    if request.user.is_authenticated:
        return render(request, "orders/fish.html", context = {"dishes":Fish.objects.all})
    else:
        return redirect("orders:login")

def gps(request):
    if request.user.is_authenticated:
        return render(request, "orders/gps.html")
    else:
        return redirect("orders:login")
...
def menu(request):
    ...
    try:
        filepath="orders\seafuu_menu.pdf"
...
def tables(request):
    if request.user.is_authenticated:
        return render(request, "orders/tables.html", context = {"tables":Table.objects.all})
    else:
        return redirect("orders:login")

def ingredient(request):
    if request.user.is_authenticated:
        return render(request, "orders/ingredient.html")
    else:
        return redirect("orders:login")
...
def checkout(request):
    if request.method == 'POST':
        ...
        table = request.POST.get('table_of_cart')
        username = request.user.username
        response_data = {}
        list_of_items = [item["item_description"] for item in cart]

        order = UserOrder(username=username, table=table, order=list_of_items, price=float(price), delivered=False) #create the row entry
