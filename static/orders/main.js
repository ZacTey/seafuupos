$(document).ready(function () {
    ...
    currenttable = localStorage.getItem('table_no');
    display_customer_table(currenttable);

    retrieve_saved_cart() //check if local storage value of "cart retrived " is True
...
function table_select(customer_table_number) {
    display_notif("table selected", customer_table_number) // Display notification

    localStorage.setItem('table_no', JSON.stringify(customer_table_number));
    newtable = localStorage.getItem('table_no'); // Assigning 
    display_customer_table(newtable)
    
}

function display_customer_table(currenttable) {
    var customertb = document.getElementById('customer-table');
    if (currenttable == null) {
        customertb.innerHTML = "No Table Selected"; //clear the table
    } else {
        customertb.innerHTML = currenttable //set the innerhtml to the input value
    }
}
...
function order_list_functionality() {
    onRowClick("orders_table", function(row) {
        ...
        //var user_is_admin = check_user_admin();
        //var user_is_kitchen = check_user_kitchen();
...
function add_to_cart(info) {
    //info will be the stuff displayed in the receipt
    // item description as well as the price
...
function display_notif(type, info = "No info provided") {
    ...
    switch (type) {
        case "add to cart":
            toastr.success(info.item_description + ': RM ' + info.price, 'Added to Cart');
            break;
        ...
        case "new order":
            ...
            break;
        case "table selected":
            toastr.success("Table " + info + " selected!");
            break;
    }
...
function load_cart() {
    ...
    if (cart !== null && cart.length > 0) {
        for (var i = 0; i < cart.length; i++) {
            ...
            item_price.innerHTML = "RM " + parseFloat(cart[i].price).toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2, minimumFractionDigits: 2 });

            total += cart[i].price
        }
        total = Math.round(total * 100) / 100
        localStorage.setItem('total_price', total);
        //localStorage.setItem('table_no', table_no); //Added by Zac
        document.getElementById('total').innerHTML = "RM " + parseFloat(localStorage.getItem("total_price")).toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2, minimumFractionDigits: 2 })

        
        onRowClick("cart_body", function(row) {
            var value = row.getElementsByTagName("td")[0].innerHTML;
            var description = row.getElementsByTagName("td")[1].innerHTML;
            var r = confirm("Proceed to delete '" + description + "' from cart?/确定删除?/Pasti Delete?");
            if (r == true) {
...
function checkout() {
    ...
    var table_of_cart = localStorage.getItem("table_no") // We need to set the local storage 
    var csrftoken = getCookie('csrftoken');
    ...
    $.ajax({
        ...
        type: "POST", // http method
        data: { cart: cart, price_of_cart: price_of_cart, table_of_cart: table_of_cart , csrfmiddlewaretoken: csrftoken }, // data sent with the post request
        ...
        error: function(xhr, errmsg, err) {
            //have this as another toast
            console.log("Your server has some issues, please check with support. . .")
